...
protocol LocationListenerDelegate {
    func didFetchLocation(lat : CLLocationDegrees , long : CLLocationDegrees)
}
...
extension WeatherForecastViewModel : LocationListenerDelegate {
    func didFetchLocation(lat: CLLocationDegrees, long: CLLocationDegrees) {
        let location = CLLocationCoordinate2D(latitude: lat, longitude: long)
        self.getWeatherForecast(locationCoordinate: location)
    }
}
extension WeatherForecastViewModel {
    ...
    func setUpLocation() {
        LocManager.shared.delegate = self
        LocManager.shared.startTracking()
